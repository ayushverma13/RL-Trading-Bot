#####________ source env/bin/activate ________#####
...
from bin.common_funcs import get_logistic_func, get_unit_sigmoid_func 

# no __init__ as dataclass decorator handles it on its own 
@dataclass
class Process1:
    @dataclass
    class State:
    ...
    # the : here is used for type checking , i.e the state arguement is of object type State class , 
    # and after -> you specify the return type , i.e up_prob takes 2 arguements , self , and state( of type state object ) , 
...
def plot_single_trace_all_processes(
    ...
    from bin.plot_funcs import plot_list_of_curves

    traces_len = len(process1_trace)

    plot_list_of_curves(
        list_of_x_vals = [range(traces_len)] * 3, # 3 time the entire range as 3 diff processes
        list_of_y_vals = [process1_trace, process2_trace, process3_trace], # list of all the Y vals 
        list_of_colors = ["r", "b", "g"],
        ...
        x_label = "Time Steps",
        y_label = "Stock Price",
        title = "Single-Trace Simulation for Each Process"
    )
